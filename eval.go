...
func (f functionkek) eval(r Row) (Value, error) {
	args := make([]Value, len(f.args))
	for i, argExpression := range f.args {
		exprResult, err := argExpression.eval(r)
		if err != nil {
...
func (e columnRef) eval(x Row) (Value, error) {
...
func (e fbinaryOr) eval(x Row) (Value, error) {
	a, err := e.left.eval(x)
	if err != nil {
		return Value{}, err
	}
...
func (e feq) eval(x Row) (Value, error) {
	a, err := e.left.eval(x)
	if err != nil {
		return Value{}, err
	}
...
func (c Value) eval(x Row) (Value, error) {
	return c, nil
}
